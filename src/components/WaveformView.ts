...
export function WaveformView({ track, isRecording, isSelected, zoom, onUpload, onShift }: WaveformViewProps) {
  const { isRecording: globalIsRecording, selectedTrackId, zoom: globalZoom } = useStudioStore(); // Get global state from Zustand
  const containerRef = useRef<HTMLDivElement>(null);
  const wavesurferRef = useRef<WaveSurfer | null>(null);

  const isThisTrackRecording = globalIsRecording && selectedTrackId === track.id;
  const isThisTrackSelected = selectedTrackId === track.id;
  ...
  useEffect(() => {
    if (containerRef.current && track.url) {
      wavesurferRef.current = WaveSurfer.create({
        ...
        normalize: true,
        interact: false,
        minPxPerSec: globalZoom, // Use globalZoom from Zustand
      });
  ...
  }, [track.url, track.color, globalZoom]); // Depend on globalZoom

  return (
    <div className={cn(
      "h-[100px] border-bottom border-studio-border relative transition-colors overflow-hidden waveform-container",
      isThisTrackSelected ? "bg-studio-panel/20" : "bg-transparent", // Use isThisTrackSelected
      isThisTrackRecording && "bg-studio-record/5" // Use isThisTrackRecording
    )}>
      <div 
            ...
            {track.type === 'instrumental' ? (
              ...
              <div className="flex flex-col items-center gap-2">
                <div className={cn(
                  ...
                  isThisTrackRecording ? "border-studio-record bg-studio-record/10 recording-glow" : "" // Use isThisTrackRecording
                )}>
                  <Mic size={18} className={isThisTrackRecording ? "text-studio-record" : "text-studio-muted"} /> // Use isThisTrackRecording
                </div>
      ...
      {isThisTrackRecording && ( // Use isThisTrackRecording
